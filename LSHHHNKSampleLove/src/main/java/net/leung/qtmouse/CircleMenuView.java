...
public class CircleMenuView extends FrameLayout {
    ...
    private static final int DEFAULT_BUTTON_SIZE = 60;
    private static final float DEFAULT_DISTANCE = DEFAULT_BUTTON_SIZE * 1.5f;
    private static final float DEFAULT_RING_SCALE_RATIO = 1.1f;
    ...
    public static class EventListener {
         ...
         */
        public void onButtonPressDown(@NonNull CircleMenuView view, int buttonIndex) {
        }
         ...
         * @param buttonIndex - clicked button zero-based index.
         */
        public void onButtonPressUp(@NonNull CircleMenuView view, int buttonIndex) {
        }
    }
    ...
    private class OnButtonTouchListener implements View.OnTouchListener {
        ...
        public boolean onTouch(View view, MotionEvent event) {
            //由无障碍助手模拟的点击需要被忽略
            if (event.getToolType(0) == MotionEvent.TOOL_TYPE_UNKNOWN) return false;

            if (!mIsAnimating) {
                switch (event.getAction()) {
                    case MotionEvent.ACTION_DOWN:
                        if (mListener != null) {
                            mListener.onButtonPressDown(CircleMenuView.this, mButtons.indexOf(view));
                        }
                        break;
                    case MotionEvent.ACTION_CANCEL:
                    case MotionEvent.ACTION_UP:
                        if (mListener != null) {
                            mListener.onButtonPressUp(CircleMenuView.this, mButtons.indexOf(view));
                        }
                        break;
                }
            }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int size = mDesiredSize;
        if (small)
            size /= 3;

        final int w = resolveSizeAndState(size, widthMeasureSpec, 0);
        final int h = resolveSizeAndState(size, heightMeasureSpec, 0);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        mMenuButton.getMeasuredContentRect(mButtonRect);
    ...
    private void initButtons(@NonNull Context context, @NonNull List<Integer> icons, @NonNull List<Integer> colors) {
        ...
        for (int i = 0; i < buttonsCount; i++) {
            ...
            button.setOnTouchListener(new OnButtonTouchListener());
            button.setScaleX(0);
            button.setScaleY(0);
    ...
    private void offsetAndScaleButtons(float centerX, float centerY, float angleStep, float offset, float scale) {
        for (int i = 0, cnt = mButtons.size(); i < cnt; i++) {
            ...
            button.setScaleX(0.8f * scale);
            button.setScaleY(0.8f * scale);
        }
    }
    ...
    private void openOrClose(boolean open, boolean animate) {
        ...
        if (animate) {
            ...
            final int visibility = open ? View.VISIBLE : View.GONE;
            for (View view : mButtons) {
